...
import canvas_harmonica

#------------------------------------------------------------------------------

class ComposantHarmonica:
	
	def __gerer(self, widget, data):
		tracer("ComposantHarmonica", "__gerer")
	...
	def update(self, ens):
		tracer("ComposantHarmonica", "update")
		ens = Ensemble("", ens)
	...
	def __init__(self, notes):
		tracer("ComposantHarmonica", "__init__")
		self.liste = ListeHarmonicas()
		self.noms = self.liste.getListeNoms()
		instrument = self.liste.getInstrument(self.noms[0])
		self.canvas = canvas_harmonica.CanvasHarmonica(instrument)
		lisPRE = ['0','1', '2']
		lisNBR = ['1','2']
		...
		labelTI, self.comboTI = definirLabelCombo(TXT_TYP_INS, self.liste.getListeNoms())
